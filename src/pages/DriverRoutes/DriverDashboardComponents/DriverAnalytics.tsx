...
export default function DriverAnalytics() {
  ...
  const avgIncome = report.totalCompletedRides
    ? (report.totalEarnings / report.totalCompletedRides).toFixed(2)
    : 0
    ...
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="bg-blue-500 text-white shadow-lg rounded-none">
          <CardContent>
            <h2 className="text-sm sm:text-base opacity-80">Total Earnings</h2>
        ...
        <Card className="bg-purple-500 text-white shadow-lg rounded-none">
          <CardContent>
            <h2 className="text-sm sm:text-base opacity-80">Avg Income/Ride</h2>
            <p className="text-xl sm:text-2xl md:text-3xl font-bold">${avgIncome}</p>
          </CardContent>
        </Card>
      </div>
        ...
        <CardContent className="px-2">
          <div className="flex justify-center items-center">
            <h2 className="text-lg font-semibold mb-4 mt-4 uppercase">
              {period.charAt(0).toUpperCase() + period.slice(1)} Rides
            </h2>
          </div>
